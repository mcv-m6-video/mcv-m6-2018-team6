...
class Original: 
    def __init__(self,name,im_dir,gt_dir,color=False,rho=0,alpha=1.8):
        self.color = color
        self.name = name
        self.im_dir = im_dir
        self.gt_dir = gt_dir   
...
class gaussian1D(Original):
    ...
    def get_motion_adaptative(self,test,image,th,rho):
        im=image
        im = np.asarray(im)
        y = np.abs(im-self.mean) >= self.th*(self.std+2)
        self.mean = (1 - self.rho) * self.mean + self.rho * im
        self.std = np.sqrt((1 - self.rho) * np.power(self.std, 2) + self.rho * np.power(im - self.mean, 2))
        return y
    ...
    def evaluation_metrics(groundtruth, prediction):
        groundtruth_aux = np.array(groundtruth)[:, :, :, 0].flatten()
        pixels = np.nonzero(np.any([(groundtruth_aux!= 85),(groundtruth_aux!= 170)], axis=0))
        trueArray = (groundtruth_aux == 255)
    
        predArray = np.array(prediction).flatten()
        precision, recall,f1_score,support = PRFmetrics(trueArray[pixels], predArray[pixels], average='binary')
